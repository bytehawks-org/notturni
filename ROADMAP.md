...
| Dashboard di amministrazione con tutte le funzionalità di moderazione | 🟡 | Da app Next.js separata dal frontend pubblico (`frontend/admin/`, servizio proprio in `compose.yaml`/`k8s/`, come backend/frontend/moderation), non più una route di `frontend/`. Tre sezioni, ognuna con un campo di ricerca: Utenti (ruoli/attivazione — nascosta in modalità `solo`, un solo utente già Super Admin), Pagine statiche, Blog (elenco piattaforma con proprietario/visibilità, sospensione — `Blog.is_suspended`, blocca lettura e scrittura pubbliche indipendentemente da `visibility`, vedi `app/domain/authorization.py`). L'editor delle pagine statiche in questa vista è una textarea Markdown semplice, non il `RichTextEditor` Tiptap del frontend pubblico (contenuto identico, solo editing meno ricco in questa vista di backoffice). **Manca ancora un pannello di moderazione trasversale dei commenti** (segnalazioni su tutti i blog) — oggi la moderazione commenti resta solo per-blog, ad opera del Mediatore di quel blog. |
